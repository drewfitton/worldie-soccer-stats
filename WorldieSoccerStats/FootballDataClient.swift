...
public struct FootballDataClient {
    ...
    public func fetchMatches(competitionId: Int, filterOption: FilterOption = .latest) async throws -> [Match] {
        let url = baseURL + "/competitions/\(competitionId)/matches?\(filterOption.urlQuery)"
        let urlRequest = URLRequest(url: URL(string: url)!)
        let response: MatchesResponse = try await fetchData(request: urlRequest)
        guard let matches = response.matches else {
            throw "Matches not found"
        }
        return matches
    }
